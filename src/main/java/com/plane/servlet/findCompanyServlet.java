...
import com.plane.entity.company;
...
@WebServlet(name = "findCompanyServlet", value = "/findCompanyServlet")
public class findCompanyServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        flightService fs = new flightServiceImpl();

        List<company> company = fs.findCompany();

        // 将其转化为json数据 序列化
        String jsonUm = JSON.toJSONString(company);
